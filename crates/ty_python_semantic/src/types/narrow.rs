...
struct PatternSuccessResult<'db> {
    ...
    /// The type safe to assign to a capture or alias after the pattern has finished evaluating.
    ///
    /// These differ for mutable sequences: a sequence pattern temporarily establishes its length
    /// and indexed element types, but later mutation can invalidate those facts. Subject narrowing
    /// does not infer bindings and leaves this equal to [`Self::matched_subject_ty`].
...
impl<'db, 'ast> NarrowingConstraintsBuilder<'db, 'ast> {
    ...
    fn evaluate_negative_pattern_predicate_kind(
        ...
        match pattern_predicate_kind {
            ...
            PatternPredicateKind::Class(_) => PatternNarrowingResult::Possible(
                self.evaluate_negative_match_pattern_class(subject, pattern_predicate_kind),
            ),
            PatternPredicateKind::Mapping(_) => PatternNarrowingResult::Possible(
                self.evaluate_negative_match_pattern_mapping(subject, pattern_predicate_kind),
            ),
...
impl<'db> PatternSuccessAnalyzer<'db> {
            ...
            PatternPredicateKind::As(pattern, name) => {
                        ...
                        .and_then(|name| self.places().symbol_id(name.as_str()))
                {
                    Self::merge_binding(
                        &mut result.bindings,
                        place.into(),
                        PatternBindingTypes::subject(result.stable_subject_ty, subject_ty),
                    );
                }
                result
            }
    ...
    fn analyze_successful_class_pattern(
        ...
        self.analyze_pattern_subject_arms(
            ...
            |analyzer, original_subject_ty, subject_ty| {
                ...
                let mut stable_subject_ty = narrowed_subject_ty;
                let mut bindings = BTreeMap::new();
                    ...
                    .zip(arguments)
                {
                    ...
                    if argument.source != PatternValueSource::Subject {
                        ...
                        stable_subject_ty = child.stable_subject_ty;
                    }
        ...
        self.analyze_pattern_subject_arms(
            ...
            |analyzer, original_subject_ty, subject_ty| {
                ...
                Some(PatternSuccessResult {
                    ...
                    stable_subject_ty: narrowed_subject_ty,
                    bindings,
                    typevar_evidence: evidence.finish(),
                })
            },
    ...
    fn analyze_successful_sequence_pattern(
        ...
        self.analyze_pattern_subject_arms(
            ...
            |analyzer, original_subject_ty, subject_ty| {
                ...
                let mut stable_element_types = analyzer
                    .infers_bindings()
                ...
                for (pattern, element_ty) in kind.patterns.iter().zip(element_types) {
                    ...
                    if let Some(stable_element_types) = &mut stable_element_types {
                ...
                let stable_subject_ty = stable_element_types.map_or(matched_subject_ty, |types| {
                    if subject_ty.exact_tuple_instance_spec(analyzer.db).is_some() {
                        analyzer.intersect_types(
                            subject_ty,
                            analyzer.successful_sequence_pattern_type(kind, &types),
                        )
                    } else {
                        analyzer.intersect_types(
                            subject_ty,
    ...
    fn analyze_pattern_subject_arms(
        ...
        let mut filtering_stable_subject_types =
        ...
        for expanded_subject_ty in expanded_subject_types {
            ...
            if let Some(stable_subject_types) = &mut filtering_stable_subject_types {
                stable_subject_types.add_in_place(expanded_result.stable_subject_ty);
        ...
        if self.infers_bindings() {
            let filtering_stable_subject_ty = filtering_stable_subject_types
                .map_or(filtering_matched_subject_ty, UnionBuilder::build);
            result.stable_subject_ty = self.typevar_preserving_intersection(
                subject_ty,
                filtering_stable_subject_ty,
                &typevars,
            );
            ...
            result.stable_subject_ty = result.matched_subject_ty;
        }
        result
    }
    ...
    fn analyze_pattern_subject_arms_impl(
        ...
        let mut matched_subject_types = UnionBuilder::new(self.db);
        let mut stable_subject_types = self.infers_bindings().then(|| UnionBuilder::new(self.db));
        let mut bindings = BTreeMap::new();
        let mut typevar_evidence = PatternTypeVarEvidence::default();

        for (original_subject_ty, arms) in &grouped_arms {
            let mut matched_types = UnionBuilder::new(self.db);
            let mut stable_types = self.infers_bindings().then(|| UnionBuilder::new(self.db));
            ...
            for (_, filtering_subject_ty) in arms {
                if let Some(arm) = analyze_arm(self, original_subject_ty, filtering_subject_ty) {
                    ...
                    if let Some(stable_types) = &mut stable_types {
            ...
            if let (Some(stable_subject_types), Some(stable_types)) =
                (&mut stable_subject_types, stable_types)
            {
        ...
        PatternSuccessResult {
            matched_subject_ty,
            stable_subject_ty: stable_subject_types.map_or(matched_subject_ty, UnionBuilder::build),
            bindings,
            typevar_evidence,
        }
    }
...
impl<'db> NarrowingConstraintsBuilder<'db, '_> {
    ...
    fn evaluate_negative_match_pattern_class(
        &mut self,
        subject: Expression<'db>,
        pattern: &PatternPredicateKind<'db>,
    ) -> Option<NarrowingConstraints<'db>> {
        ...
        if definitely_matched.is_never() {
            // A class pattern like `case Point(x=0, y=0)` is not irrefutable, so a failed match
            // cannot exclude `Point` as a possibility.
            return None;
        }
    ...
    fn evaluate_negative_match_pattern_mapping(
        &mut self,
        subject: Expression<'db>,
        pattern: &PatternPredicateKind<'db>,
    ) -> Option<NarrowingConstraints<'db>> {
