...
pub(super) fn structured_sections(source: &str) -> Vec<Section> {
    ...
    google::visit_sections(source, |kind, range, body| {
        if let Some(section) = google_section(kind, range, body) {
...
fn google_section(kind: SectionKind, range: TextRange, body: &[ParsedLine<'_>]) -> Option<Section> {
    let items = match kind {
        SectionKind::Returns | SectionKind::Yields => parse_return_item(kind, body),
        _ => parse_named_items(kind, body, |line| parse_named_item(kind, line)),
    }?;
...
fn parse_return_item(kind: SectionKind, body: &[ParsedLine<'_>]) -> Option<Vec<SectionItem>> {
    ...
    if !first_is_preformatted {
        preformatted.observe_non_preformatted_line(first_line.text);
    }
    ...
    for line in lines {
        ...
        preformatted.observe_non_preformatted_line(line.text);
        item.push_description(line.text);
    }
